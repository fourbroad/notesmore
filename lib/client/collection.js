...
inherits(Collection, Document, {
  ...
  distinctQuery: function(field, opts, callback) {
    this.getClient().emit('distinctQueryCollection', this.domainId, this.id, field, opts, function(err, data) {
      if (err) return callback ? callback(err) : console.error(err);
      callback(null, data);      
    });
  },
  ...
  bulk: function(docs, callback){
    this.getClient().emit('bulk', this.domainId, this.id, docs, function(err, result){
      if(err) return callback ? callback(err) : console.error(err);
      callback ? callback(null, result) : console.log(result);
    });
  },
