<template>
  <div class="view-container">
          ...
          <div class="actions btn-group float-right">
            ...
            >
              ...
              <ul class="dropdown-menu dropdown-menu-right">
                ...
                <li
                  class="dropdown-item export-csv-all"
                  @click="exportCsv('allColumns')"
                >{{$t('exportAllCSV')}}</li>
                <li
                  class="dropdown-item export-csv-current"
                  @click="exportCsv('visibleColumns')"
                >{{$t('exportCurrentCSV')}}</li>
                <div class="dropdown-divider"></div>
                <template v-for="act in localeActions">
                  <li
                    class="dropdown-item"
                    v-if="act.id!=actionId"
            ...
            <div class="dropdown-menu dropdown-menu-right">
              ...
              <ul class="item-container scrollbar fa-ul mx-1 mb-0">
                ...
                >
                  <i
                    class="fa-li fa"
                    v-bind:class="{'fa-square-o': sf.visible==false, 'fa-check-square-o': sf.visible==undefined||sf.visible}"
                  ></i>
                  {{sf.title }}
                </li>
              </ul>
    ...
    <div class="row">
      <div
        class="col-md-12"
      >{{$t('tableHint',{start:from+1, end:from+documents.length, total:total})}}</div>
    </div>
    <div class="row">
      <div class="col-md-12">
        <div class="table-responsive">
          <table class="view-table table table-striped table-hover table-sm">
              ...
              <tr>
                ...
                <template v-for="(column, colIndex) in i18n_columns">
                  ...
                  >
                    ...
                    <i
                      class="fa"
                      :class="{'fa-sort':column.sortable&&!column.order, 'fa-sort-desc':column.order=='desc', 'fa-sort-asc': column.order=='asc'}"
                    ></i>
                  </th>
                </template>
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                      ...
                      <ul class="item-container scrollbar fa-ul mx-1 mb-0">
                        ...
                        >
                          <i
                            class="fa-li fa"
                            v-bind:class="{'fa-square-o': col.visible==false, 'fa-check-square-o': col.visible==undefined||col.visible}"
                          ></i>
                          {{col.title }}
                        </li>
                      </ul>
            ...
            <tbody>
              <tr
                v-for="(doc, row) in documents"
                :class="{'table-active':isActive(doc)}"
                @click="setActive(doc)"
                :key="keyRow(doc, row)"
              >
                ...
                <template v-for="(column, col) in document.columns">
                  <td
                    :key="keyCell(doc, column, row, col)"
                    v-if="column.visible==undefined||column.visible"
                  >
                    <a
                ...
                <td class="text-center" width="46px">
                  ...
                  <ul class="dropdown-menu dropdown-menu-right">
                    <template v-for="act in localeRowActions">
                      <li
                        class="dropdown-item"
                        @click="onRowActionClick(doc, act)"
                        :key="act.collectionId+'~'+act.id"
                      >{{act.title}}</li>
                    </template>
                    <li
    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <select
          class="custom-select custom-select-sm page-setting"
          style="display:inline;width:62px"
          v-model="size"
        >
            ...
            <template v-for="pageBtn in pageBtns">
              ...
              >
                <a
                  class="page-link"
                  href="javascript:void(0)"
                  :aria-disabled="pageBtn.disabled"
                >{{pageBtn.label}}</a>
              </li>
            </template>
            <li
...
export default {
  ...
  methods: {
    ...
    saveAs(id, title) {
      let view = _.cloneDeep(this.document),
        i18n = view._i18n;
      view.title = title;
      if (i18n) {
        _.each(i18n, value => {
    ...
    onSearchConfigurationItemClick(fieldName) {
      let field = this.getSearchField(fieldName),
        visible = field.visible == undefined || field.visible;
    ...
    checkRowDeleteable(row) {
      return utils
        .checkPermission(row.domainId, this.currentUser.id, "delete", row)
        .then(result => {
    ...
    exportCsv(source) {
      let doc = this.document,
        localeDoc = this.localeDoc,
        columns = localeDoc.columns,
        title = `${_.at(localeDoc, "export.prompt.export")[0] || "Exports"} ${
          localeDoc.title
        }`,
        rows = [],
        start = +moment(),
        ...
        doc[scrollId ? "scroll" : "findDocuments"](opts).then(data => {
          ...
          if (data.documents.length > 0) {
            ...
            this.updateToast({
              ...
              nonce: nonce,
              message: `${((count * 100) / data.total).toFixed(2)}% ${_.at(
                localeDoc,
                "export.prompt.completed"
              )[0] || "completed"} (${count}/${data.total}).`
            });
            ...
            this.updateToast({
              ...
              hint: `${moment.duration(moment() - start).asSeconds()}(s)`,
              nonce: nonce,
              message: `${localeDoc.title}.csv ${_.at(
                localeDoc,
                "export.prompt.hasBeenExported"
              )[0] || "has been exported!"}`
            });
          }
        });
      ...
      this.addToast({
        ...
        nonce: nonce,
        message: `${_.at(localeDoc, "export.prompt.startExporting")[0] ||
          "Start exporting"} ${localeDoc.title}`
      });
      doExport();
    },
    at(object, path) {
