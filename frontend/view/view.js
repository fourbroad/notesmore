...
$.widget("nm.view", {
  ...
  _create: function () {
    ...
    this._on(this.$saveBtn, {
      click: this._onSave
    });
    this._on(this.$cancelBtn, {
      click: this._onCancel
    });
    this._on(this.$favorite, {
  ...
  _onFavorite: function (e) {
    ...
    Profile.get(view.domainId, currentUser.id, function (err, profile) {
      ...
      let oldFavorites = _.cloneDeep(profile.favorites),
        patch,
        index = _.findIndex(profile.favorites, function (f) {
          return f.domainId == view.domainId && f.collectionId == view.collectionId && f.id == view.id;
        });
      if (index >= 0) {
        patch = [{
          op: 'remove',
  ...
  refresh: function () {
    ...
    this.table = this.$viewTable.DataTable({
      ...
      processing: true,
      serverSide: true,
      columns: _.map(_.cloneDeep(viewLocale.columns), (c) => {
        if (c && c.name && !c.data) {
          c.data = c.name
        }
        return c;
      ...
      search: {
        search: _.at(view, '.search.fulltext.keyword')[0]
      },
      ...
      headerCallback: function (thead, data, start, end, display) {
        new Vue({
          el: $(thead).find(':last-child').addClass('text-center').attr({
            is: 'ColumnSetting',
            ':columns': 'columns',
            'v-on:item-changed': 'onItemChanged'
          }).get(0),
          data: {
          ...
          methods: {
            onItemChanged: function (item) {
              let col = _.filter(o.view.columns, function (c) {
              ...
              _this.table.column(item.name + ':name').visible(item.visible == true || item.visible == undefined);
              _this.table.columns.adjust().draw(false);
              _this.refresh();
            }
          },
          components: {
  ...
  _onLoadAction: function (e) {
    ...
    this.element.trigger('actionclick', {
      dom: view.domainId,
      col: view.collectionId,
      doc: view.id,
      act: action.id
    });
  },
  ...
  _refreshSearchBar: function () {
    let o = this.options,
      ...
      view = o.view,
      viewLocale = view.get(o.locale),
      searchFields = _.filter(view.search.fields, function (sf) {
        return sf.visible == undefined || sf.visible == true;
      });

    this.$searchContainer.empty();
    _.each(searchFields, function (sf) {
      let title = _.filter(viewLocale.search.fields, function (sfl) {
        return sfl.name == sf.name
      })[0].title;
      switch (sf.type) {
        case 'keywords':
          $("<div/>").appendTo(_this.$searchContainer).keywords({
            ...
            valueChanged: function (event, values) {
              let sf2 = _.find(view.search.fields, function (o) {
                return o.name == sf.name
              });
    ...
    new Vue({
      el: $('<div class="search-item"/>').appendTo(this.$searchContainer).attr({
        is: 'SearchSetting',
        ':fields': 'fields',
        'v-on:item-changed': 'onItemChanged',
      }).get(0),
      data: {
        fields: viewLocale.search.fields
      },
      methods: {
        onItemChanged: function (item) {
          let field = _.filter(o.view.search.fields, (f)=>{
          ...
          _this._refreshSearchBar();
          _this._refreshHeader();
        }
      },
      components: {
        SearchSetting: () => import('./components/SearchSetting.vue')
      }
    });
  ...
  _armSearchCol: function () {
    ...
    return _.reduce(view.columns, function (searchCols, col) {
      let sf = _.find(view.search.fields, {
        'name': col.name
      });
      if (sf) {
        switch (sf.type) {
          ...
          case 'datetimeRange':
            searchCols.push({
              search: _this._buildDatetimeRangeSearch(sf)
            });
            break;
          case 'datetimeDuedate':
            searchCols.push({
  ...
  _refreshFavorite: function (favorites) {
    ...
    if (o.isNew) {
      ...
      function doRefreshFavorite(favorites) {
        ...
        if (_.find(favorites, function (f) {
            return f.domainId == view.domainId && f.collectionId == view.collectionId && f.id == view.id;
          })) {
      ...
      if (favorites) {
        ...
        Profile.get(view.domainId, currentUser.id, {
          refresh: true
        }, function (err, profile) {
          if (err) return console.error(err);
  ...
  _columnType: function (name) {
    let view = this.options.view;
    let index = _.findIndex(view.columns, function (sf) {
  ...
  _kvMap: function (aoData) {
    return _.reduce(aoData, function (result, kv) {
  ...
  _buildSort: function (kvMap) {
    let iSortingCols = kvMap["iSortingCols"],
      sort = new Array(iSortingCols);
    for (let i = 0; i < iSortingCols; i++) {
      let current = {},
        colName = kvMap['mDataProp_' + kvMap['iSortCol_' + i]];
  ...
  saveAs: function (id, title, callback) {
    let o = this.options,
      locale = o.locale,
      client = o.view.getClient(),
      View = client.View,
      view = _.cloneDeep(o.view),
      _this = this;
    view.title = title;
    if (o.locale) {
  ...
  showIdTitleDialog: function (options) {
    let _this = this;
    import( /* webpackChunkName: "idtitle-dialog" */ 'idtitle-dialog/idtitle-dialog').then(({
      default: itd
    }) => {
