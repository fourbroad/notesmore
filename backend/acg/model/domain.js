...
function initDomain(authorId, authorTitle, domainId) {
  return initTempates(domainId).then(() => {
    ...
    return elasticsearch.bulk({
      ...
      console.log('Collections, Metas, Views, Actions, Pages, Roles and Users created!');
    });
  });
}
...
_.assign(Domain, {
  ...
  create: function (authorId, domainId, domainData, options) {
    ...
    return promise.then(() => {
      return createEntity(elasticsearch, authorId, ROOT, DOMAINS, domainId, domainData, {
        metaIndex: ROOT + '~' + Meta.METAS + '~hot~snapshots'
      });
    }).then(data => {
  ...
  get: function (domainId, options) {
    var uid = uniqueId(ROOT, DOMAINS, domainId),
      version = _.at(options, 'version')[0],
      domain;
...
inherits(Domain, Document, {
  ...
  copy: function (authorId, targetId, targetTitle, options) {
    var self = this,
      es = this._getElasticSearch(),
      includeData = options && options.includeData;
    ...
    function copyCollection(collection, targetDomainId) {
      return Promise.all([
        collection.getSnapshotTemplate().then(template => {
          var temp = {
            aliases: {},
            index_patterns: [targetDomainId + '~' + collection.id + '~snapshots~*-*']
          };
        ...
        collection.getEventTemplate().then(template => {
          var temp = {
            aliases: {},
            index_patterns: [targetDomainId + '~' + collection.id + '~events~*-*']
          };
    ...
    return this._getElasticSearch().mget({
      body: {
        docs: _.reduce(ids, function (result, value, key) {
          result.push({
            _index: documentAllAlias(domainId, value.collectionId),
            _type: Document.TYPE,
            _id: value.id
          });
          return result;
